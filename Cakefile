# Building Kineograph requires coffee-script and uglify-js. For
# help installing, try:
#
...
javascripts = {
  'js/kineograph.js': ['coffee/kineograph.coffee']
}
...
#
write_kineograph_javascript = (filename, body, trailing='') ->
...
# Build Kineograph.
#
task 'build', 'build Kineograph from source', build = (cb) ->
  ...
  try
    for javascript, sources of javascripts
      ...
      write_kineograph_javascript javascript, code
      unless process.env.MINIFY is 'false'
        write_kineograph_javascript javascript.replace(/\.js$/,'.min.js'), (
...
task 'watch', 'watch coffee/ for changes and build Kineograph', ->
    ...
    without_existing_tag ->
      build ->
        tag_release ->
          push_repo [], ->
            push_repo ['--tags'], ->
              console.log "Successfully tagged #{version_tag()}: https://github.com/epicagency/kineograph/tree/#{version_tag()}"
            , untag_release
          , untag_release
        , untag_release